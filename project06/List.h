...
public:
	...
	bool insertAfter(const Item& oldName, const Item& newName);
	bool insertBefore(const Item& oldName, const Item& newName);
...
private:
	...
	Node* findPtrs(const Item& findItem, Node* prevPtr, Node* nextPtr);
 ...
 * @param: oldName, the name we will insert after
 * @param: newName, the name which will be inserted
 * retuns: Whether or not the operaton completed successfully
 */
template <class Item>
bool List<Item>::insertAfter(const Item& oldName, const Item& newName) {

	Node* nodePtr = myFirst;
	for (unsigned i = 0; i < mySize; i++) {
		if (nodePtr -> myItem == oldName) {
			if (i + 1 == mySize) {
				append(newName);
			} else {
				Node* newNode = new Node(newName, nodePtr -> myNext);
				nodePtr -> myNext = newNode;
				mySize++;
			}
			return true;
		}
 ...
 * @param: oldName, the name we will insert before
 * @param: newName, the name which will be inserted
 * retuns: Whether or not the operaton completed successfully
 */
template <class Item>
bool List<Item>::insertBefore(const Item& oldName, const Item& newName) {

	Node* nodePtr = myFirst;
	Node* prevPtr = myFirst;

	if (myFirst -> myItem == oldName) {
		prepend(newName);
		return true;
	}
	...
	for (unsigned i = 0; i < mySize; i++) {
		if (nodePtr -> myItem == oldName) {
			Node* newNode = new Node(newName, nodePtr);
			prevPtr -> myNext = newNode;
			mySize++;
			return true;
		}
		prevPtr = nodePtr;
...
/* Output operator
 * Outputs the list to strean out, and returns the ostream.
 */
template<class Item>
